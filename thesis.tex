...
A number of improvements have been made to garbled circuits since their inception in 1986.
...
\begin{itemize}
    \item asdf
    \item Based on point and permute, what is first ciphertext?
    \item Assume it's $C_1$, with input keys $A_0$, $B_1$.
    \item Then set $C_1 \leftarrow \Enc_{A_0, B_1}^{-1}(0^n)$
    \item So now we don't have to send the first CT: if evaluator sees a red-red, then assume $0^n$.
\end{itemize}

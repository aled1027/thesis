...
\subsection{Goldreich's Security Definition}
This is from page 620, definition 7.2.1.

This is the definition for two party MPC in the presence of static semi-honest adversaries.

Let $f: \{0,1\}^* \times \{0,1\}^* \to \{0,1\}^* \times \{0,1\}^*$ be a functionality, where $f_1(x,y)$ denotes the first element of $f(x,y)$, and likewise $f_2(x,y)$ represents the second element.
...
Define the output of the first party after an execution of $\Pi$ on inputs $(x,y)$ as $OUTPUT^{\Pi}_1(x,y)$. 

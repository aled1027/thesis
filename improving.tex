...
More formally, the garble randomly samples $b$ from $\{0,1\}$, and then gives wire label $W_i^0$ select bit $b$ and gives $W_i^1$ select bit $1-b$.
...
$A, B$ and $W_k$ are wires, where $W_i^0$ and $W_i^1$ are the zeroith and first wire label of wire $W_i$ respectively.
...
Since the evaluator does not know whether $W_k^0$ or $W_k^1$ is set to $0^n$, the evaluator does not learn any information about the semantic representation of $C_*$..
...
The garlber sets $W_k^0 = W_i^0 \oplus W_j^0$ and sets $W_k^1 = W_k^0 \oplus \Delta$.
...
\end{align}
For any wire $W_i$, since $W_i^1$ is dependent on $W_i^0$, we often simplify notation such that the wire labels for $W_i$ are $W_i$ and $A \oplus \Delta$, omitting the subscript.
...
$W_i$ has labels $W_i$ and $A \oplus \Delta_A$ and $W_j$ has labels $W_j$ and $B \oplus \Delta_B$.
To perform Free XOR, $A,B$ and $W_k$ need to be using the same delta value. 
The garbler adds an extra gate between $W_i$ and $W_j$ and the $XOR$ gate that adjusts their XOR value to the correct value.
$W_i$ and $A \oplus \Delta_A$ change to $A'$ and $A' \oplus \Delta$.
$W_j$ and $B \oplus \Delta_B$ change to $B'$ and $B' \oplus \Delta$.
Since $A, B$ and $W_k$ have the same delta value, FreeXOR is used.
...
Free XOR only requires that the three wires involved in the XOR gate, $A, B$ and $W_k$, use the same delta, so each XOR gate has its own $\Delta$.
...
\section{Half Gates}
\al{for some reason it uses hashes instead of encryption in the paper. not sure why. and in the talk, mike uses enc. in our code, alex m used hash}
...
This means that the garbler knows whether $A_*$ is $W_i^0$ or $W_i^1$.
Let $a,b$ and $c \in \{0,1\}$ represent the semantic value of $W_i$, $W_j$ and $W_k$ respectively.
If $a = 0$, then $c = a \wedge b = 0$, so the garbler creates a unary gate that always outputs $W_k^0$.
...
Since the evaluator has $B_*$, the evaluator can compute the label of the output wire of the AND gate by xoring the rows in the garbled table by $H(B)$ or $H(B \oplus \Delta)$ to get $W_k$ or $C \oplus \Delta$ respectively. 
...
\begin{table}[]
    ...
    \begin{tabular}{|c|}
        ...
        \hline
        $\Enc_{W_j^0}(W_k^0)$ \\
        $\Enc_{W_j^1}(W_k^0 \oplus a\Delta)$ \\
        \hline
    \end{tabular}
...
Otherwise if $a = 1$, then the evaluator should acquire $W_k^0 \oplus b\Delta$.
In this case, it suffices for the evaluator to obtain $\Omega = W_k^0 \oplus W_j^0$, as the evaluator simply sets $C_*$ to $B_* \oplus \Omega$ which sets $C_*$ equal to $W_k^0 \oplus b\Delta$.

...
She starts with a typical boolean circuit, like the one shown in figure \ref{fig:basic-circuit} \al{add figure of basic circuit with gates labeled}. 
...
After assigning labels to $W_0^0$, $W_0^1$, $W_1^0$, $W_1^1$, $W_5^0$ and $W_5^1$, Alice creates a garbled table $T_{G_0}$ for the gat.e
...
And the fourth column is the dual-key cipher encryption\footnote{See section \al{what section?} for information on dual-key ciphers} of the value in the third column, using the values of the first two columns as keys.
